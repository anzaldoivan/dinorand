...
public sealed class RandomizerConfig
{
    ...
    /// <summary>
    /// DC1 only (on by default). Patch <c>DINO.exe</c> so a newly mapped room-to-room door transition
    ...
    /// and reversed by <c>--restore</c>. Disable at install time with the CLI <c>--no-dc1-door-skip</c>.
    /// When no selected install executable exists, the room overlay remains usable and this patch is skipped.
    /// Like <see cref="Dc1CutsceneSafeEnemies"/>, NOT seed-encoded.
    /// </summary>
    public bool Dc1DoorSkip { get; set; } = true;
