...
// ImplicitUsings pulls in System.IO, whose Path collides with the WPF-style shape Path.
using Path = Avalonia.Controls.Shapes.Path;

namespace DinoRand.App
{
    /// <summary>
    /// Avalonia port of the WPF <c>PieChart</c>. The algorithm (pie geometry, bar packing,
    /// luma-based label colour) is copied verbatim; only the framework API surface is
    /// translated. Translation notes vs the WPF original:
    ///   • <c>UIElement</c> → <see cref="Control"/>
    ///   • <c>Visibility.Visible/Collapsed</c> → <c>IsVisible = true/false</c>
    ///   • <c>x.ToolTip = v</c> → <c>ToolTip.SetTip(x, v)</c>
    ///   • <c>new LineSegment(pt, false)</c> / <c>new ArcSegment(...)</c> ctor args
    ///     → object initialisers (Avalonia segments are parameterless + properties)
    ///   • <c>PathGeometry.Figures</c> / <c>PathFigure.Segments</c> assigned explicitly.
    /// </summary>
    public partial class PieChart : UserControl
    {
        // viewBox / grid / topGrid are the x:Name'd controls; their fields and the
        // InitializeComponent() that wires them are emitted by Avalonia's XAML name generator.
        ...
        public void UpdatePie()
        {
            ...
            var gridItems = new List<Control>();
            ...
            if (records.Length == 1)
            {
                ...
                ToolTip.SetTip(circle, GetRecordToolTip(record, total));
            ...
            else
            {
                ...
                for (int i = 0; i < records.Length; i++)
                {
                    ...
                    pathFigure.Segments.Add(new LineSegment { Point = piePoint });
                    pathFigure.Segments.Add(new ArcSegment
                    {
                        Point = nextPiePoint,
                        Size = new Size(radius, radius),
                        RotationAngle = 0,
                        IsLargeArc = angleLength >= Math.PI,
                    ...
                    ToolTip.SetTip(path, GetRecordToolTip(record, total));
                    gridItems.Add(path);
        ...
        private Grid CreateHorizontalBar(Record[] records)
        {
            ...
            foreach (var record in records)
            {
                ...
                ToolTip.SetTip(textBlock, GetRecordToolTip(record, total));
