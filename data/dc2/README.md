...
|---|---|---|
| `enemies.json` | 13 `E*.DAT` model slots w/ obj/tri/quad counts **+** 11-creature roster | model counts = **bytes**; creature mapping = runtime-confirmed except EA0 = high inference |
...
| `item-placements.json` | per-room SAT-1 trigger commits (op 0x31): VM-thread slot + routine + blob offset, 168 triggers / 54 rooms (K81) | **bytes** (static slot-5 decode; 3 live-validated); not catalog-item identity or an editable item lever |
...
All slots except EA0 are creature-mapped by runtime capture: E00=Velociraptor, E10=Tyrannosaurus,
E20=Pteranodon, E30=Plesiosaurus boss, E31/E32=Plesiosaurus regular/grunt forms,
E40=Giganotosaurus, E50=Inostrancevia, E60=Allosaurus, E70=Triceratops, E80=Mosasaurus, and
E90=Oviraptor. EA0=Compsognathus remains the single high-confidence rig-size inference. The
earlier E30/E31/E32 Velociraptor-triple inference is superseded: live captures place all raptor
colours on E00. See `enemies.json` and EXE-ENEMY-TABLE K68/K70.
...
2. ~~**All numeric item ids.**~~ **RESOLVED (K75):** the full 58-entry catalog (`0x704260`)
   with English names is in `items.json` — ids 0x00–0x39 incl. key items 0x21–0x34.
3. ~~**Per-room door connections & exact spawn/item placements.**~~ **RESOLVED (K78/K81/K99):**
...
placements K99, flag gating K107, conditional commits K119, and enemy mapping K68/K70): the
remaining honest gaps are the ST-id→room-name mapping and the runtime-seeded consumable pickup
ids above. Section 1 of the placeholder list is still live; treat it as scaffolding, and the
rest as byte-backed ground truth.
